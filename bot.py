...
async def login_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Create a new event loop for this user
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        telethon_loops[user_id] = loop
        
        # Store phone in config
...
async def perform_login(user_id, api_id, api_hash, phone, chat_id):
    """Perform actual login with new session"""
    try:
        # Delete old session file to avoid conflicts
        ...
        if not await client.is_user_authorized():
            try:
                # Send OTP with proper hash
                result = await client.send_code_request(phone)
                
                # Store phone_code_hash for later verification
...
async def login_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Complete login in background with proper sign in
        def complete_login():
            try:
                loop = telethon_loops.get(user_id, asyncio.new_event_loop())
                asyncio.set_event_loop(loop)
                loop.run_until_complete(verify_otp_v2(user_id, client, phone, otp, phone_code_hash, update.message.chat_id))
            except Exception as e:
...
async def verify_otp_v2(user_id, client, phone, otp, phone_code_hash, chat_id):
    """Verify OTP using proper SignInRequest"""
    try:
        # Try to sign in with the OTP
        try:
            # Use direct SignInRequest for better control
            ...
            logger.info(f"✅ SignIn successful for {user_id}")
            ...
            send_status(user_id, "error", {"error": "Invalid OTP code! Please try again."})
            await context.bot.send_message(
                chat_id=chat_id,
            ...
            if "PHONE_CODE_EXPIRED" in str(e) or "EXPIRED" in str(e):
                # OTP expired - resend
                send_status(user_id, "status", {"message": "OTP expired! Sending new one..."})
                try:
                    result = await client.send_code_request(phone)
                    ...
                    send_status(user_id, "error", {"error": f"Failed to resend OTP: {str(e2)}"})
                    return
            else:
                send_status(user_id, "error", {"error": str(e)})
                return
