...
def set_client(user_id, client):
    with telethon_lock:
        telethon_clients[user_id] = client
        logger.info(f"✅ Client stored for user {user_id}")
...
async def complete_login(user_id, client, phone):
    """Complete login and store client"""
    try:
        me = await client.get_me()
        
        # IMPORTANT: Store client BEFORE starting background thread
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.info(f"Clients: {list(telethon_clients.keys())}")
