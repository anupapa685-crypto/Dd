...
# Telethon Clients Storage - PERMANENT
...
def set_client(user_id, client):
    with telethon_lock:
        telethon_clients[user_id] = client
        logger.info(f"✅ Client stored permanently for user {user_id}")
...
def get_loop(user_id):
    with telethon_lock:
        return telethon_loops.get(user_id)

def set_loop(user_id, loop):
    with telethon_lock:
        telethon_loops[user_id] = loop

def remove_loop(user_id):
    with telethon_lock:
    ...
    try:
        ...
        if not await client.is_user_authorized():
            try:
                ...
                send_status(user_id, "error", {"error": f"Wait {e.seconds} seconds"})
            except Exception as e:
...
async def complete_login(user_id, client, phone):
    """Complete login and store client permanently"""
    try:
        me = await client.get_me()
        
        # IMPORTANT: Store client FIRST
        ...
        loop = get_loop(user_id) or asyncio.get_event_loop()
        ...
        def run_client():
            try:
                ...
                logger.error(f"Client run error for {user_id}: {e}")
        ...
        logger.info(f"✅ User {user_id} logged in permanently as {me.first_name}")
...
async def login_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        def verify():
            try:
                loop = get_loop(user_id) or asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
        ...
        await update.message.reply_text(
            f"<b>⏳ Still verifying... Check /status</b>",
            parse_mode='HTML'
        )
        return ConversationHandler.END
...
def main():
    try:
        ...
        logger.info("🔐 Permanent Login System: Active")
